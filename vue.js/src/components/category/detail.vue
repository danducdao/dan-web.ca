  ...
  methods: {
    ...
    onSubmit: function() {
      if (this.$route.params.id) {
        ...
        this.categorieService
          .updateCategorie(this.$route.params.id, this.model)
          .then(data =>
            this.responseSaveCategorie(
              this,
              this.produitService,
              data.body,
              this.model
        ...
        this.categorieService
          .saveCategorie(obj)
          .then(
            data => (data ? this.$router.push({ name: "ListeCategorie" }) : "")
          );
      }
    },
    ...
    responseSaveCategorie(obj, produitService, OldCategorie, NewCategorie) {
      ...
      produitService.produitListe().then(function(produits) {
        var produitBD = produits.body.filter(
          produit => produit.category[0]._id == OldCategorie._id
        );
