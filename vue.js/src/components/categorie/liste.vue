...
export default {
    ...
    methods : {
        ...
        removeCategorie(categorieId)
        {
            ...
            {
                this.categorieService.removeCategorieById(categorieId).then(data => this.responseRemoveCategorie(this,data.body));
            }
        },
        responseRemoveCategorie(obj,oldCategorie)
        {
            if(oldCategorie)
            {
                ...
                obj.categorieService.categorieListe().then( data => obj.responseCategorieList(obj,data.body,oldCategorie));
            }else{
        ...
        responseCategorieList(obj,categories,oldCategorie)
        {
            ...
            obj.produitService.produitListe().then(data => obj.suppimerProduit(obj,data.body,oldCategorie)); 
        },
        suppimerProduit(obj,produitBD,oldCategorie)
        {
            var produits= produitBD.filter(data => data.category[0]._id == oldCategorie._id);
