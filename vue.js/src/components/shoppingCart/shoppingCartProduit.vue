...
export default {
  ...
  created() {
    this.shoppingCartService
      .shoppingCartListe()
      .then(data => this.response(this, data.body));
  ...
  methods: {
    shoppingCartList(categorieId) {
      return this.shoppingCartData.filter(
        produit => produit.category[0]._id.indexOf(categorieId) !== -1
      );
    },
    ...
    onSubmit(event, shoppingCartId) {
      ...
      var carts = [];
      var item = this.shoppingCartData.filter(
