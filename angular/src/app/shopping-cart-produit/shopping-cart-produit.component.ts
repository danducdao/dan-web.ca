  ...
  template: `
    <article>
     <div class="row show-grid">
         <div class="col-sm-2" style="max-height:190px;"*ngFor="let produit of this.produits" >
                   ...
                   <div><input type="checkbox" name="produit" [value]="produit._id" (change)="selectProduit($event)" ></div>
         </div>
     </div>
    </article>
  `,
...
export class ShoppingCartProduitComponent implements OnInit {
    ...
    ngOnInit() {
         this._produitService.getProduitList()
                             .subscribe(data => this.produits = data.filter((data:IProduit) => data.category[0]._id.indexOf(this.categorieId) !== -1));
    }
    changeQty(event:any){
    ...
    updateQty(value:string,id:string,qte:string,index:number){
